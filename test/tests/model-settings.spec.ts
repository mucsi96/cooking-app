...
test('accepts fenced recipe JSON and rejects malformed fenced JSON', async ({ page }) => {
  await page.goto('/importalas');
  await page.getByLabel('Recept szövege').fill('INVALID_JSON');
  ...
  await page.getByLabel('Recept szövege').fill('FENCED_JSON');
  await page.getByRole('button', { name: 'Importálás' }).click();
